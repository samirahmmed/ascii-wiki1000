...
const ART_STYLES = [
  'Minimalist', 'Detailed', 'Blocky', 'Abstract', 'Pixelated', 'Line Art', 
  'Grunge', 'Glitch', 'Blueprint', 'Calligraphy', 'Doodle', 'Hatching', 
  'Stipple', '8-Bit', 'Halftone', 'Typographic', 'Circuit Board', 'Origami', 
  'Mosaic', 'Sketch', 'Stencil', 'Psychedelic', 'Art Deco', 'Gothic', 'Vaporwave'
];
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initializeAds = async () => {
      // AdMob will only be available on native platforms
      if (Capacitor.isNativePlatform()) {
        try {
          // Dynamically import AdMob to prevent browser crashes
          const { AdMob } = await import('@capacitor-community/admob');
          await AdMob.initialize({
            requestTrackingAuthorization: true,
            initializeForTesting: true, // Use testing config during development
          });
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    try {
      const artPromise = generateAsciiArt(query, language, apiKey, artStyle).catch(err => {
  ...
  }, [language, translations, apiKey, artStyle]);
  ...
  const handleSave = () => {
    if (!topic || !content || !asciiArt) return;
    ...
    link.href = url;
    link.download = `ASCII-Wiki-${topic.replace(/\s+/g, '_')}.txt`;
